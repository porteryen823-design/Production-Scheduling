...
class SchedulerFullExample:
    ...
    def run(self):
        ...
        # 計算機台群組利用率
        self.calculate_machine_group_utilization(results_new)
    ...
    def calculate_machine_group_utilization(self, results: List[Dict]):
        if not results:
            print("無結果，無法計算利用率")
            return
        ...
        for i, (group, (util, used, capacity)) in enumerate(sorted_groups[:5]):
            print(f"{i+1}. {group} | Utilization: {util:.2%} | Used: {used:.1f} min | Capacity: {capacity:.1f} min")
