...
function createRouterError(type, params) {
  if (false) return assign(new Error(ErrorTypeMessages[type](params)), {
    type,
...
function extractComponentsGuards(matched, guardType, to, from, runWithContext = (fn) => fn()) {
  ...
  for (const record of matched) {
    if (false) warn$1(`Record with path "${record.path}" is either missing a "component(s)" or "children" property.`);
...
function useHistoryStateNavigation(base) {
  ...
  function changeLocation(to, state, replace$1) {
    ...
    try {
      ...
      if (false) warn$1("Error with push/replace State", err);
  ...
  function push(to, data) {
    ...
    if (false) warn$1("history.state seems to have been manually replaced without preserving the necessary values. Make sure to preserve existing history state if you are manually calling history.replaceState:\n\nhistory.replaceState(history.state, '', url)\n\nYou can find more information at https://router.vuejs.org/guide/migration/#Usage-of-history-state");
...
function createWebHashHistory(base) {
  ...
  if (false) warn$1(`A hash base must end with a "#":
...
function tokenizePath(path) {
  ...
  if (!path.startsWith("/")) throw new Error(false ? `Route paths should start with a "/": "${path}" should be "/${path}".` : `Invalid path "${path}"`);
...
function createRouterMatcher(routes, globalOptions) {
  ...
  function addRoute(record, parent, originalRecord) {
    ...
    if (false) checkChildMissingNameWithEmptyPath(mainNormalizedRecord, parent);
    ...
    for (const normalizedRecord of normalizedRecords) {
      ...
      if (false) throw new Error('Catch all routes ("*") must now be defined using a param with a custom regexp.\nSee more at https://router.vuejs.org/guide/migration/#Removed-star-or-catch-all-routes.');
      matcher = createRouteRecordMatcher(normalizedRecord, parent, options);
      if (false) checkMissingParamsInAbsolutePath(matcher, parent);
      if (originalRecord) {
        originalRecord.alias.push(matcher);
        if (false) checkSameParams(originalRecord, matcher);
      } else {
        originalMatcher = originalMatcher || matcher;
        if (originalMatcher !== matcher) originalMatcher.alias.push(matcher);
        if (isRootAdd && record.name && !isAliasRecord(matcher)) {
          if (false) checkSameNameAsAncestor(record, parent);
  ...
  function resolve(location$1, currentLocation) {
    ...
    if ("name" in location$1 && location$1.name) {
      ...
      if (false) warn$1(`The Matcher cannot resolve relative paths but received "${path}". Unless you directly called \`matcher.resolve("${path}")\`, this is probably a bug in vue-router. Please open an issue at https://github.com/vuejs/router/issues/new/choose.`);
...
function findInsertionIndex(matcher, matchers) {
  ...
  if (insertionAncestor) {
    ...
    if (false) warn$1(`Finding ancestor route "${insertionAncestor.record.path}" failed for "${matcher.record.path}"`);
  }
  return upper;
}
...
function createRouter(options) {
  ...
  if (false) throw new Error('Provide the "history" option when calling "createRouter()": https://router.vuejs.org/api/interfaces/RouterOptions.html#history');
  ...
  function addRoute(parentOrRoute, route) {
    ...
    if (isRouteName(parentOrRoute)) {
      ...
      if (false) warn$1(`Parent route "${String(parentOrRoute)}" not found when adding child route`, route);
      record = route;
  ...
  function removeRoute(name) {
    ...
    else if (false) warn$1(`Cannot remove non-existent route "${String(name)}"`);
  }
  function getRoutes() {
...
- Location:`, rawLocation);
    ...
    if (rawLocation.path != null) {
      if (false) warn$1(`Path "${rawLocation.path}" was passed with params but they will be ignored. Use a named route alongside params instead.`);
    ...
    if (false) warn$1(`A \`hash\` should always start with the character "#". Replace "${hash}" with "#${hash}".`);
 ...
 Are you always returning a new location within a navigation guard? That would lead to this error. Only return when redirecting or aborting, that should fix this. This might break in production if not fixed.`);
  ...
  function triggerError(error, to, from) {
    ...
    if (list.length) list.forEach((handler) => handler(error, to, from));
      ...
      if (false) warn$1("uncaught error during route navigation:");
      console.error(error);
    }
    ...
    install(app) {
      ...
      if (isBrowser && !started && currentRoute.value === START_LOCATION_NORMALIZED) {
        ...
        push(routerHistory.location).catch((err) => {
          if (false) warn$1("Unexpected error when starting the router:", err);
        });
      }
      const reactiveRoute = {};
      ...
      if (__VUE_PROD_DEVTOOLS__ && isBrowser) addDevtools(app, router, matcher);
    }
  };
