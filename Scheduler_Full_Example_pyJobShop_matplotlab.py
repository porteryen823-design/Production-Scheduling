...
from pyjobshop import ProblemData, Solution, TaskData, Job, Task, Mode, Machine
...
class SchedulerFullExample:
    ...
    def run(self):
        ...
        # 使用 plot_machine_gantt 繪製甘特圖
        if results_new:
            self.plot_machine_gantt(results_new)
            self.plot_task_gantt(results_new)
    ...
    def plot_machine_gantt(self, results: List[Dict]):
        """
        使用 pyjobshop 的 plot_machine_gantt 繪製機器甘特圖
        """
        # 建立資源列表 (機器)
        all_machines = set()
        for r in results:
            all_machines.add(r["Machine"])
        all_machines = sorted(list(all_machines))

        resources = []
        machine_to_idx = {}
        for idx, machine_name in enumerate(all_machines):
            resources.append(Machine(name=machine_name))
            machine_to_idx[machine_name] = idx
        ...
        for r in results:
            lot_id = r["LotId"]
            if lot_id not in lot_to_job_idx:
                job_idx = len(jobs)
                lot_to_job_idx[lot_id] = job_idx
                jobs.append(Job(name=lot_id, tasks=[]))

            job_idx = lot_to_job_idx[lot_id]
            task_idx = task_counter
            task_counter += 1

            # 建立任務
            task = Task(name=f"{lot_id}_{r['Step']}", job=job_idx)
            tasks.append(task)

            # 建立模式 (每個任務只有一個模式，使用指定的機器)
            machine_idx = machine_to_idx[r["Machine"]]
            duration = int((datetime.fromisoformat(r["End"].replace('Z', '')) -
                           datetime.fromisoformat(r["Start"].replace('Z', ''))).total_seconds() / 60)
            mode = Mode(task=task_idx, resources=[machine_idx], duration=duration)
            modes.append(mode)

            # 更新 job 的任務列表
            jobs[job_idx].tasks.append(task_idx)
        ...
        for r in results:
            lot_id = r["LotId"]
            task_name = f"{lot_id}_{r['Step']}"
            task_idx = next(i for i, t in enumerate(tasks) if t.name == task_name)

            start_time = int((datetime.fromisoformat(r["Start"].replace('Z', '')) - self.SCHEDULE_START).total_seconds() / 60)
            end_time = int((datetime.fromisoformat(r["End"].replace('Z', '')) - self.SCHEDULE_START).total_seconds() / 60)

            task_data = TaskData(
                mode=0,  # 每個任務只有一個模式
                resources=[machine_to_idx[r["Machine"]]],
                start=start_time,
                end=end_time
            )
            task_data_list.append(task_data)

        solution = Solution(problem_data, task_data_list)
        ...
        plt.title("Machine Gantt Chart")

        # 設定 x 軸為實際時間
        ax.xaxis.set_major_formatter(mdates.DateFormatter('%m-%d %H:%M'))
        ax.xaxis.set_major_locator(ticker.MaxNLocator(10))  # 限制最多10個標記

        # 將 x 軸刻度從分鐘轉換為日期時間
        ticks = ax.get_xticks()
        new_labels = [self.SCHEDULE_START + timedelta(minutes=t) for t in ticks]
        ax.set_xticks(ticks)
        ax.set_xticklabels([dt.strftime('%m-%d %H:%M') for dt in new_labels])
        ...
        gantt_dir = r"C:\Data\APS\Gantt"
        os.makedirs(gantt_dir, exist_ok=True)
        plt.savefig(os.path.join(gantt_dir, "machine_gantt_pyjobshop.png"), dpi=300, bbox_inches='tight')
        plt.show()

    def plot_task_gantt(self, results: List[Dict]):
        """
        使用 pyjobshop 的 plot_task_gantt 繪製任務甘特圖
        """
        # 建立資源列表 (機器)
        all_machines = set()
        for r in results:
        ...
        fig, ax = plt.subplots(figsize=(15, 10))
        plot_task_gantt(solution, problem_data, ax=ax)
        ...
        gantt_dir = r"C:\Data\APS\Gantt"
        os.makedirs(gantt_dir, exist_ok=True)
        plt.savefig(os.path.join(gantt_dir, "task_gantt_pyjobshop.png"), dpi=300, bbox_inches='tight')
        plt.show()
