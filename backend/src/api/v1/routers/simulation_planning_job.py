"""
SimulationPlanningJob API 路由
"""
from fastapi import APIRouter, Depends, HTTPException, Query
from sqlalchemy.orm import Session
from typing import List
from infra.db.database import get_db
from domain.models import SimulationPlanningJob, DynamicSchedulingJob
from core.config import settings
from api.v1.schemas.simulation_planning_job import (
    SimulationPlanningJobCreate,
    SimulationPlanningJobResponse
)

router = APIRouter(prefix="/simulation-planning-jobs", tags=["SimulationPlanningJobs"])


@router.get("", response_model=List[SimulationPlanningJobResponse])
def get_simulation_planning_jobs(
    ...
    """取得所有模擬規劃備份"""
    jobs = db.query(SimulationPlanningJob).order_by(SimulationPlanningJob.CreateDate.desc()).offset(skip).limit(limit).all()
    return jobs


@router.post("/save", status_code=201)
def save_current_job_to_simulation(
    job_info: SimulationPlanningJobCreate,
    db: Session = Depends(get_db)
):
    ...
    # 3. 執行批量插入
    try:
        db.bulk_insert_mappings(SimulationPlanningJob, data_to_save)
        db.commit()
...
def load_simulation_job(
    ...
    target_backup = db.query(SimulationPlanningJob).filter(SimulationPlanningJob.id == job_id).first()
    if not target_backup:
        raise HTTPException(status_code=404, detail=f"找不到 ID 為 {job_id} 的模擬規劃")
    
    # 根據 key_value 找出同一批的所有備份
    all_backups = db.query(SimulationPlanningJob).filter(SimulationPlanningJob.key_value == target_backup.key_value).all()
...
def delete_simulation_job(job_id: int, db: Session = Depends(get_db)):
    ...
    target = db.query(SimulationPlanningJob).filter(SimulationPlanningJob.id == job_id).first()
    if not target:
        raise HTTPException(status_code=404, detail=f"找不到 ID 為 {job_id} 的模擬規劃")
    
    # 刪除同一批 key_value 的所有記錄
    db.query(SimulationPlanningJob).filter(SimulationPlanningJob.key_value == target.key_value).delete()
    db.commit()
    return None
